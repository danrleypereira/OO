...
public class AnaliseDados {
	...
	*/
	public static void main(String[] args) {
		...
		ArrayList<Pessoas> conjuntoPessoas = new ArrayList<Pessoas>();
...
//		Pessoas[] conjuntoPessoas = (Pessoas[]) conjuntoPessoasList.toArray();
//		
		
		povoarArray(conjuntoPessoas, NUMERO_PESSOAS);
		
		for(int i=0; i< NUMERO_PESSOAS; i++){
			pesosOrdem[i] = conjuntoPessoas[i].getPeso();
		}
	...
	public static void povoarArray(Pessoas[] conjuntoPessoas, int numeroPessoas){
		for(int i = 0; i < numeroPessoas; i++){
			conjuntoPessoas[i].inserirSexo();
			conjuntoPessoas[i].inserirPeso(i);
		}
	}
